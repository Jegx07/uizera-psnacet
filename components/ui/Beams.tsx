...
function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: any): THREE.ShaderMaterial {
  const physical = THREE.ShaderLib.physical;
  const { vertexShader: baseVert, fragmentShader: baseFrag, uniforms: baseUniforms } = physical;
  const baseDefines = (physical as THREE.ShaderLibShader & { defines?: Record<string, unknown> }).defines ?? {};
  ...
  Object.entries(cfg.uniforms ?? {}).forEach(([key, u]) => {
    (uniforms as Record<string, { value: unknown }>)[key] = u !== null && typeof u === "object" && "value" in (u as object)
      ? (u as { value: unknown })
      : { value: u };
  });
